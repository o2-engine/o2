...
Flies the actor along a spline trajectory. The spline (key ranges give a random offset corridor) is geometrically mapped between the start and finish points; progress is driven by the animatable `position` property 0..1 (`SetPosition`); the trajectory point is written into the actor transform (widgets get layout offsets) immediately when the property is set — including editor scrubbing — and on each update. The random offset within the corridor is the `randomOffset` property (0..1, visible in the editor); it rerolls when `position` enters zero (flight start), by an explicit `ResetRandomOffset()` or by writing the property; mid-range scrubbing never changes the trajectory; a clip re-evaluation for a dependent animation (sub-track particle baking replays `position` from zero, `IAnimation::IsEvaluatingSubControlOwner()`) does not count as a start. The finish can be moved mid-flight (`SetPoints`) — the offset is kept and the flight homes onto the new point without a jump. `alignToDirection` turns the actor along the trajectory tangent, `directionAngleOffset` (degrees) says which way the sprite faces (−90 for one drawn pointing up). The spline is edited in the scene with the spline tool when the component is selected
