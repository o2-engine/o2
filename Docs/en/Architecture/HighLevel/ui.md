...
The widget pivot (`SetPivot2D`) is the only base-transform field the layout persists: it cannot be derived from the rectangle, rotation happens around it and child actors are placed from it; the default (0, 0) is not written.
