...
Используется для доступа к структуре типов в рантайме (поиск анимируемых значений по строковому пути, вызов коллбеков по имени, сериализация/десериализация, автоматические биндинги скриптов). В основе лежит метаинформация о типах, генерируемая утилитой **o2CodeTool**. Она парсит заголовочные файлы (`.h`), находит в них классы C++ и анализирует их структуру (включая наследование). Затем определяет наследников `IObject` и в конце `.h` генерирует метаинформацию о типе. Утилита токенизирует заголовок и разбирает поток токенов быстрым рекурсивным спуском (это не полноценный парсер C++ — распознаются только нужные рефлексии конструкции). Файлы перезаписываются только если сгенерированная метаинформация действительно изменилась, а каждый проект экспортирует граф разобранных типов в `CodeToolCache.xml`, который зависимые проекты подгружают для разрешения базовых классов между проектами.
...
Утилитарный слой также включает:

- [Математика](/Docs/ru/Architecture/Utils/math.md) — векторы (2D/3D), матрицы, прямоугольники, кватернионы, `o2::Transform`.
- [Контейнеры](/Docs/ru/Architecture/Utils/containers.md) — `o2::Vector`, `o2::Map` и другие обёртки над STL с расширенным API.
- [Строки](/Docs/ru/Architecture/Utils/string.md) — `o2::String`, конвертация типов, форматирование.
- [Файловая система](/Docs/ru/Architecture/Utils/filesystem.md) — синглтон `o2FileSystem`: файлы, папки, пути.
- [Логирование](/Docs/ru/Architecture/Utils/logging.md) — иерархия каналов `o2::LogStream`.
- [Отладка](/Docs/ru/Architecture/Utils/debug.md) — `o2Debug`: лог и отрисовка отладочных примитивов.
- [Задачи](/Docs/ru/Architecture/Utils/tasks.md) — `o2Tasks`: отложенные и покадровые задачи.
- [Время и таймеры](/Docs/ru/Architecture/Utils/time.md) — `o2Time`, `Timer`, `TimeStamp`.
...
Как было описано выше это входная точка и основная система движка. На этапе инициализации она инициализирует другие подсистемы. Далее начинается игровой цикл, который обрабатывается в функции ProcessFrame(). Там вызывается апдейт сцены, инпута и всех остальных подсистем, а так же происходит рендер кадра. Сам жизненный цикл выполняется как корутина: `ProcessFrame()` продвигает её по одному кадру за раз, выполняя разовую стадию загрузки, а затем покадровое обновление и отрисовку. Так же в обертке приложения обрабатываются системные сообщения: активация и деактивация, и т.д.
...
Рендеринг может идти параллельно: главный поток записывает команды отрисовки, а выделенный поток рендера отправляет их на GPU, синхронизируясь каждый кадр. Включён там, где это поддерживает платформа (сейчас macOS / Metal); в остальных случаях главный поток отправляет отрисовку напрямую.
...
Сверху бекенда реализована движковая обёртка, доступная через синглтон `o2Scripts`, и враппер JavaScript-переменных `o2::ScriptValue`.  
...
## Звуки [(подробная документация)](/Docs/ru/Architecture/LowLevel/sound.md)

Звуком управляет подсистема `o2::SoundSystem` (синглтон `o2Sounds`) на базе miniaudio. Звук проигрывается через `o2::SoundPlayer` — наследник `o2::IAnimation`, поэтому его можно секвенировать и скраббить в редакторе анимаций. На сцене используются компоненты `o2::SoundComponent` и `o2::SoundListenerComponent`, ассет — `o2::SoundAsset` (wav, ogg, mp3, flac). Поддерживается пространственный звук: слушатель следует за камерой либо за активным компонентом-слушателем.

## Сеть [(подробная документация)](/Docs/ru/Architecture/LowLevel/network.md)

Сетевая система `o2::NetworkSystem` (синглтон `o2Network`) предоставляет HTTP-клиент и TCP/UDP-сокеты. У каждой асинхронной операции есть форма с коллбеком и форма-корутина, все коллбеки вызываются из покадровой прокачки на главном потоке. HTTP-запросы идут через платформенный бэкенд (NSURLSession, HttpURLConnection, браузерный fetch) либо портируемый поверх сокетов, с движковыми cookies, кешем ответов и редиректами. Над сырыми сокетами есть уровень сообщений с фреймингом по длине: `TcpMessageChannel`/`TcpMessageServer` для чатов и лобби, а `UdpSocket` закрывает обмен в реальном времени для онлайн-геймплея. HTTP-сокращения и классы сокетов также привязаны в скрипты (`o2.Http`, `o2.TcpMessageChannel`, `o2.UdpSocket`).

## Пайплайны [(подробная документация)](/Docs/ru/Editor/Pipeline/internals.md)

AI-пайплайны контента - подсистема только редактора: ассеты `.pipeline`, реестр нод, исполнитель и клиенты провайдеров лежат в `o2/Editor/Sources/o2Editor/Pipeline` и в рантайм не входят. Сборка игры считает файлы `.pipeline` обычными бинарными ассетами. Устройство и окно Pipeline описаны в документации редактора.
...
Акторы могут содержать список компонентов, определяющих их логику и отрисовку. Для создания пользовательской компоненты нужно унаследовать её от `o2::Component` и реализовать нужный функционал (`OnStart`, `OnEnabled/Disabled`, `OnDraw`, `OnUpdate` и т. д.). Такой подход позволяет реализовывать логику по принципу композиции: один базовый актор может включать несколько компонентов. [Список встроенных компонентов](/Docs/ru/Architecture/HighLevel/components.md).
