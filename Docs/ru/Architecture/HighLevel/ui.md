...
Поворот, масштаб и сдвиг (`angle`, `scale`, `shear` трансформации) действуют на виджет поверх лэйаута: сам лэйаут остаётся осевым прямоугольником в «лэйаут-пространстве» (`GetWorldRect`), а мировой базис (`GetWorldBasis`) — это прямоугольник с применённым поворотом/масштабом виджета и всех родителей. Слои и дочерние виджеты размещаются через это отображение (`WidgetLayout::GetLayoutToWorldBasis`), хит-тест слоёв переводит точку обратно (`WorldToLayoutPoint`). Ограничение: область отсечения `ScrollArea` остаётся осевой. Лэйауты сценических виджетов округляются до целых единиц (пиксельная точность); редактор отключает округление в режиме правки (`WidgetLayout::SetSceneLayoutsRounding`), чтобы рамка двигалась плавно при любом зуме.

Pivot виджета (`SetPivot2D`) — единственное поле базового transform, которое лэйаут сохраняет в файл: он не выводится из прямоугольника, а вокруг него идёт поворот и от него считаются позиции детей-акторов; по умолчанию (0, 0) не пишется.
...
Прозрачность задается на виджете и распространяется на его слои, а так же на все дочерние виджеты и так далее. Итоговая прозрачность слоя записывается в альфу цвета drawable, поэтому полупрозрачность задаётся прозрачностью виджета или слоя, а не альфой цвета спрайта (её перезапишет любой апдейт прозрачности).
