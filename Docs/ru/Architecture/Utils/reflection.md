...
- `ReferenceType` - тип для умных ссылок `Ref<>`, может вернуть тип объекта ссылки
...
В классах, которые отражены в рефлексии, добавляется статичное поле `o2::Type* type`, в котором хранится тип класса. Так же добавляется **не статичный** метод `const o2::Type& GetType() const`, который в рантайме возвращает реальный тип объекта.

Так же доступен макрос `TypeOf(TYPE)`, который возвращает тип по любому C++ типу. Это может быть как класс, так и элементарный тип int, float, bool и др.

### Кодогенерация и препроцессор
Генератор пишет мета-данные в конец `.h`, а регистратор `DECLARE_CLASS` - в парный `.cpp`. Без этого регистратора статичное поле `type` остается нулевым, и `GetType()` возвращает нулевую ссылку - поэтому класс, помеченный `IOBJECT`, отражается всегда, даже если цепочка его базовых классов проходит через файлы, которых генератор не видит (например, исключенные); такой случай сопровождается предупреждением.

Класс внутри блока условной компиляции получает мета-данные, обернутые в то же условие. Include guard (`#ifndef NAME`, сразу за которым идет `#define NAME`) условием не является: мета-данные размещаются после его `#endif`, и оборачивание отключило бы их, поэтому такие guard'ы распознаются и игнорируются.
