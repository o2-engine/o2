...
Макросы работают и в классах вне `namespace o2` — все имена внутри них квалифицированы полностью. Так объявлены свойства у нод cocos2d в интеграции: поверх собственных пар `set`/`get`, что даёт и короткий API (`node->rotation = 45`), и строки в редакторе, идущие через сеттеры.
