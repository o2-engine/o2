...
`Math::Random(min, max)` берёт значение из текущего источника: по умолчанию это глобальный `rand()`.
`Math::RandomScope scope(seed)` на время своей жизни переключает `Math::Random` на собственный
детерминированный генератор (minstd) — вложенные скоупы восстанавливают предыдущий, глобальный
`rand()` не сбрасывается и не расходуется. Так эмиттер частиц запекает кадры воспроизводимо
(`seed + номер кадра`), не ломая случайность остального кода (`Math::RandomUnit()` — число 0..1 из
текущего источника)

`Math::Random()` без аргументов даёт 64-битное число из генератора с сидом на процесс: из него берутся
id акторов и компонентов, поэтому id, созданные в разных запусках, не повторяются
