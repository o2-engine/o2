...
`Editor::PipelineNodeRegistry` хранит все `Editor::PipelineNodeSchema` (`type`, `label`, `category`, `description`, `inputs`, `outputs`, `addableInputs`, `instant`, `perPortRun`, `defaultSize`) вместе с реализацией (`Editor::PipelineNodeBase::Run` — `Coroutine<PipelineRunResult>`, получающая контекст выполнения и значения входов). `AllSchemas()`, `GetSchema(type)`, `CreateNode(type, position)`, `IsFinishType`. Кроме `Run` реализация может переопределить `InitNode(node)` (конфиг, с которым создаётся нода), а для ноды с `perPortRun` - `SyncPorts(node)` (она сама владеет списком выходов, и `SyncNodeWithSchema` не трогает выходы схемы), `PortCacheExcludedKeys()` и `PortCacheVariant(node, portId)` - ключи конфига, которые выбрасываются из попортовой сигнатуры, и то, что добавляется в неё для конкретного порта.

`Editor::PipelineRegions` - список частей ноды вырезания: `PipelineExtractRegion` (`id` - id выходного порта с этой частью, `name` - что вырезать и имя порта, нормализованная рамка), `Read`/`Write` по конфигу ноды (нода, сохранённая до появления частей, читается как одна часть из `prompt` и `roi`), `PortNames` (уникальные, непустые), `SyncPorts` (пересобирает выходы, сохраняя id, поэтому связи переживают переименование), `RegionOfPort`, `autoSplitPrompt` и `ParseAutoSplit(answer)` (модель отвечает `box_2d` в виде `[ymin, xmin, ymax, xmax]` на сетке 0..1000).

Категории: источники (`sourceText`, `sourceImage`, `sourceAudio`), преобразования (`textCompose`, `textConcat`, `drawImage`, `imageOutline`, `imageShadow`, `imageGradient`, `imageColor`, `audioProcess`), AI (`aiText`, `textEdit`, `promptGen`, `nanoBananaGen`, `imageEdit`, `imageExtract`, `removeBackground`, `aiRemoveBg`, `videoGen`, `sfxGen`, `ttsSpeech`, `musicGen`), выходы (`composer`, `finishImage`, `finishText`, `finishVideo`, `finishAudio`). Ноды с `instant` выполняются локально и автоматически перезапускаются редактором.
...
Помощники для картинок: `Editor::PipelineImageOps` (масштаб, `Fit` - вписать в размер с сохранением пропорций на прозрачном холсте, обрезка, обрезка по содержимому, хромакей, двухпроходная маска, обводка, тень, градиент, цветокоррекция, отражение, поворот, nine-slice, `ComposeLayers`, `Blank`, доступ `Pixel` — битмапы RGBA, `Pixel(bmp, x, y)` в координатах картинки), `Editor::PipelineTransparency` (пост-шаги белый/чёрный и хромакей), `Editor::PipelineAudio`. `ResolveComposerLayers(node)` перечисляет слои композера от нижнего к верхнему (входы-картинки, дубликаты из `dupLayers`, порядок из `layerOrder`).

## Выполнение

`Editor::PipelineExecutor::Execute(pipelineId, graph, targetNodeId, dirtyNodeIds, cachedOnly)` выполняет цепочку выше целевой ноды в порядке зависимостей. Результаты кешируются в `Work/Pipelines/cache/<pipelineId>/{content,previews,ran}` по сигнатуре ноды: нода с совпавшей сигнатурой берётся с диска, `cachedOnly` берёт ноды провайдеров из кеша или, если его нет, из последнего отрисованного превью (без отметки о запуске) и отказывает только нодам без того и другого (`Not cached`). `ResolveSourceValue(node, assetsPath, value, error)` (`Nodes/PipelineNodesCommon.h`) - единственное место, где нода-источник превращает конфиг в значение: через него работают сами источники, и им же редактор даёт источникам значение без запуска. `PipelineUtils::PrettyModelName(id)` переводит id моделей в названия для списков моделей. `PipelineGraph::GetRunTargets()` возвращает цели запуска всего графа: все ноды без исходящих связей, финишные первыми, без источников, которые никто не читает. Нода с `perPortRun` считается выход за выходом: производятся только порты, которые читает ветка (целевая нода считает все свои), каждый со своей сигнатурой (общий конфиг без `PortCacheExcludedKeys` плюс `PortCacheVariant`) и своим превью (`GetPortPreviewPath`, `LoadPortPreview`, событие `NodeOutput` с `portId`), поэтому упавшая или изменённая часть не трогает остальные; потребитель такого выхода хешируется по той части, которую читает (`PipelineGraph::ComputePortSignature`, `OutputSigKey`), так что две одинаково настроенные ноды на двух частях не делят ячейку кеша. `ExecuteSingle` выполняет одну ноду, `Cancel()` останавливает после текущей, `assetsPathOverride` перенаправляет финишные ноды (тесты). События (`Editor::PipelineExecEvent`): `NodeState` (`queued`, `running`, `done`, `error`), `NodeOutput` (`Editor::PipelineValue` с `previewPath`/`srcPreviewPath`), `Retry`, `Log`, `Done`, `Fatal`. `LoadPreview` и `ClearNodeCache` управляют кешем из редактора.

Финишные ноды пишут результат в `<assets>/<assetPath>.<ext>` (при необходимости обрезанный и масштабированный; `keepAspect` вписывает картинку в `resizeW`x`resizeH`, а не растягивает её) и ставят `assetsChanged`; в редакторе исполнитель затем вызывает `o2Assets.RebuildAssets()`.
...
## Импорт
`PipelineImport::ParseBytes` читает экспорт AssetsLine по содержимому: JSON пайплайна, JSON-бандл с результатами в data URL (`Parse`) или ZIP-бандл (`ParseZip`: `pipeline.json`, необязательный `manifest.json` со списком `results/<файл>` по нодам с типом и mime - запись с ключом `<nodeId>#<portId>`, называющая оба, это одна часть попортовой ноды - и `uploads` с файлами, которые читают ноды-источники; файлы вне манифеста берутся по имени `results/<nodeId>.<ext>`) в `PipelineGraph` со свежим `id` и карту результатов `PipelineValue`; `StoreResults` записывает их как превью, записи кеша содержимого и отметки актуальности для id пайплайна - сначала загрузки, потому что нода-источник хеширует то, что читает, затем части попортовой ноды под их собственными превью портов и сигнатурами (`PipelineExecutor::PortSignature`); `ImportFile` создаёт ассет в папке, сохраняет результаты под id графа и пересобирает ассеты. `PipelineZip` - небольшой читатель и писатель архивов под этим (записи stored и deflate, zlib).

`O2_PIPELINE_RUN=<файл>` прогоняет ассет `.pipeline` без редактора - тестом `PipelineRunAsset.RunFile` из `o2EditorTests`: выполняются все цели запуска графа с настоящими провайдерами, поэтому арт-пайплайн можно гонять из командной строки вместо окна Pipeline (`O2_PIPELINE_NODE` ограничивает запуск одной нодой, `O2_PIPELINE_ASSETS` задаёт папку, куда пишут финишные ноды, `O2_PIPELINE_WORK` - где лежит кеш, `O2_PIPELINE_TIMEOUT` - бюджет одной цели в секундах).
