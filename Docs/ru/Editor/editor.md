...
### Стиль UI редактора
Внешний вид виджетов редактора строится кодом (`EditorUIStyleBuilder`, `EditorUIStyle.cpp`) и кэшируется прототипами в `o2/Editor/Assets/Editor UI styles` (не в git). Кэш загружается только если его собранная копия в `BuiltAssets/<platform>/EditorData` сгенерирована из текущего `EditorUIStyle.cpp`; иначе редактор при старте пересобирает стиль в памяти (около 0.1 с) и обновляет сохранённые прототипы, которые попадут в собранную копию при следующей сборке таргета `Editor`.
